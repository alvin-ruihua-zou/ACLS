...
LACLS::LACLS(DiscreteSpaceInformation *space)
    : m_space(space),
      m_open(),
      m_attractors(),
      m_states(),
...
void LACLS::printState(int state_id)
{
...
void LACLS::removeAttractor(int attr_id)
{
...
int LACLS::getExpansions()
{
    return expansions;
}

/// Set the goal state.
int LACLS::set_goal(int goal_state_id)
{
...
/// Set the start state.
int LACLS::set_start(int start_state_id)
{
...
LACLS::SearchState *LACLS::getSearchState(int state_id)
{
...
LACLS::SearchState *LACLS::createState(int state_id)
{
...
void LACLS::initSearchState(SearchState *state)
{
...
LACLS::Attractor *LACLS::createAttractor(SearchState *s)
{
...
double LACLS::computeFval(SearchState *s) const
{
    return s->g + s->h * m_weight;
}

bool LACLS::almostEqual(double c1, double c2)
{
    if (fabs(c1 - c2) < 1e-9)
    {
        return true;
...
void LACLS::expand(SearchState *s)
{

    m_succs.clear();
    m_costs.clear();
    m_action_ids.clear();
    bool best_pred_match;

    m_space->GetSuccsAndBestPred(s->state_id, s->action_operators, s->attr_id, s->bp_idx, &best_pred_match, &m_succs, &m_costs, &m_action_ids);
    if (best_pred_match && m_space->GetAttractorDist(s->state_id, s->bp_attr_id) < m_dist_thresh)
    {
        s->attr_id = s->bp_attr_id;
    }
    else
    {
        s->attr_id = s->bp_idx;

        if (s->bp_idx >= m_attractors.size())
        {
            m_attractors.resize(s->bp_idx + 1, nullptr);
        }
        auto itr = m_attractors[s->bp_idx];
        if (itr == NULL)
        {

            Attractor *attr = new Attractor;
            m_space->createAttractor(s->state_id, s->bp_idx);
            attr->attr_id = s->bp_idx;
            attr->pred_attr_id = s->bp_attr_id;
            attr->counter = 1;
            m_attractors[s->bp_idx] = attr;
        }
        else
        {
            itr->counter += 1;
    ...
    for (size_t idx = 0; idx < m_succs.size(); ++idx)
    {
        ...
        if (new_cost < succ_state->g)
        {
            ...
            succ_state->bp_idx = s->state_id;
            succ_state->bp_attr_id = s->attr_id;
            succ_state->attr_id = s->attr_id;
            m_space->UpdateBP(succ_state_id, s->state_id);
        ...
        else if (almostEqual(new_cost, succ_state->g))
        {

            int id = m_space->getMatchingState(succ_state->state_id, succ_state->attr_id, succ_state->bp_idx, s->state_id, s->attr_id);
            if (id == s->state_id)
            {
                m_attractors[succ_state->attr_id]->counter -= 1;
                m_attractors[s->attr_id]->counter += 1;
    ...
    // // Decrease counter by one when deleting the state
    m_attractors[s->attr_id]->counter -= 1;
...
int LACLS::getPath(SearchState *goal_state)
{
    ...
    while (!m_open.empty())
    {
        ...
        if (min_state->state_id == goal_state->state_id || min_state->g == goal_state->g)
        {
            ...
            m_action_ids.clear();
            bool best_pred_match;
            m_space->GetSuccsAndBestPred(goal_state->state_id, goal_state->action_operators, goal_state->attr_id, goal_state->bp_idx, &best_pred_match, &m_succs, &m_costs, &m_action_ids);

            if (best_pred_match)
            {
                goal_state->attr_id = goal_state->bp_attr_id;
            }
            else
            {
                goal_state->attr_id = goal_state->bp_idx;

                if (goal_state->bp_idx >= m_attractors.size())
                {
                    m_attractors.resize(goal_state->bp_idx + 1, nullptr);
                }
                auto itr = m_attractors[goal_state->bp_idx];
                if (itr == NULL)
                {

                    Attractor *attr = new Attractor;
                    m_space->createAttractor(goal_state->state_id, goal_state->bp_idx);
                    attr->attr_id = goal_state->bp_idx;
                    attr->pred_attr_id = goal_state->bp_attr_id;
                    attr->counter = 1;
                    m_attractors[goal_state->bp_idx] = attr;
                }
                else
                {
                    itr->counter += 1;
...
int LACLS::plan(int startID, int goalID, vector<int> *solution, int *cost)
{
    ...
    init_attr->counter = 1;
    m_space->UpdateBP(start_state->state_id, startID);
    m_space->createAttractor(start_state->state_id, startID);
    ...
    start_state->attr_id = startID;
    start_state->bp_attr_id = startID;
    start_state->bp_idx = startID;
