...
std::vector<TileMap> readTileMapsFromFile(const std::string &filename)
{
    ...
    std::string line;
    int row_size = 4; // 4x4 grid for 15-puzzle
    ...
    while (std::getline(file, line))
    {
        ...
        if (tileMap.size() != row_size * row_size)
        {
            throw std::runtime_error("Invalid TileMap size in file.");
        }
...
int main()
{
    ...
    string filename = "15 tile map.txt";
    tile_maps = readTileConfigurations(filename);

    // tile_maps = readTileMapsFromFile("tiles_100_korf.txt");
    ...
    // }
    // getc(stdin);
    test_maps(tile_maps, true, true, true, true, true);
