...
{
    template <>
    struct hash<Grid2D::GridState>
    {
        typedef Grid2D::GridState argument_type;
        typedef std::size_t result_type;

        result_type operator()(const argument_type &s) const
        {
            std::size_t seed = 0;
            boost::hash_combine(seed, s.coord.first);
            boost::hash_combine(seed, s.coord.second);
            return seed;
        }
    };
}
...
public:
    ...
    typedef Grid2D::GridState StateKey;
    hash_map<StateKey *, int, Grid2D::StateHash, Grid2D::StateEqual> m_state_to_id;

    // maps from stateID to coords
    std::vector<Grid2D::GridState *> m_states;     ///< Maps state IDs to GridState pointers.
    std::vector<Grid2D::GridState *> m_attractors; ///< List of attractor states.

    // Frontier Search
    typedef unordered_map<int, Grid2D::GridState *> frontier_midpoints_t;
    ...
    // SMGS
    vector<Grid2D::GridState *> m_ssp; ///< Sparse solution path states.
     ...
     */
    Grid2D::GridState *getHashEntry(int state_id) const;
     ...
     */
    int getHashEntry(const Grid2D::RobotCoord &coord);
     ...
     */
    int createHashEntry(const Grid2D::RobotCoord &coord);
     ...
     */
    int getOrCreateState(const Grid2D::RobotCoord &coord);
     ...
     */
    void setGoalState(const Grid2D::RobotCoord &coord);

    /**
     * @brief Sets the start state.
     * @param coord The coordinate of the start state.
     */
    void setStartState(const Grid2D::RobotCoord &coord);
     ...
     */
    bool coordFree(const Grid2D::RobotCoord &coord);
     ...
     * @param attr_id The ID of the attractor.
     */
    double GetAttractorDist(const Grid2D::RobotCoord &coord, int attr_id);
     ...
     */
    bool extractPath(string search, const std::vector<int> &ids, std::vector<Grid2D::GridState *> &path);
};
