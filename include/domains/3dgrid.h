#ifndef GRID_3D_ENVIRONMENT_H_
#define GRID_3D_ENVIRONMENT_H_
...
{
    template <>
    struct hash<Grid3D::GridState>
    {
        typedef Grid3D::GridState argument_type;
...
public:
    ...
    typedef Grid3D::GridState StateKey;                                             ///< Alias for the state key type.
    hash_map<StateKey *, int, Grid3D::StateHash, Grid3D::StateEqual> m_state_to_id; ///< Maps GridState pointers to state IDs.

    std::string m_planner; ///< Name of the planner being used.

    std::vector<Grid3D::GridState *> m_states;     ///< Maps state IDs to GridState pointers.
    std::vector<Grid3D::GridState *> m_attractors; ///< List of attractor states.

    typedef unordered_map<int, Grid3D::GridState *> frontier_midpoints_t;
    ...
    std::vector<Grid3D::GridState *> m_ssp; ///< List of sparse solution points.
     ...
     */
    Grid3D::GridState *getHashEntry(int state_id) const;
     ...
     */
    void setGoalState(const Grid3D::Robot3DCoord &coord);
     ...
     */
    void setStartState(const Grid3D::Robot3DCoord &coord);
     ...
     */
    bool extractPath(string search, const std::vector<int> &ids, std::vector<Grid3D::GridState *> &path);
};

#endif // GRID_3D_ENVIRONMENT_H_